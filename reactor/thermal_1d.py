...
class ThermalModel1D(nn.Module):
    ...
    def __init__(self, config, num_nodes=50, device='cpu', dtype=torch.float64):
        ...
        self.register_buffer('h_film_boiling', torch.tensor(3.0, device=device, dtype=dtype))
        ...
        # === Параметры кипения ===
        self.register_buffer('T_sat', torch.tensor(345.0, device=device, dtype=dtype)) 
        self.register_buffer('LatentHeat', torch.tensor(100.0, device=device, dtype=dtype)) 
        
        # Обратные связи (из конфига)
        self.register_buffer('alpha_fuel', torch.tensor(config.temp_coeff_fuel, device=device, dtype=dtype)) 
        self.register_buffer('alpha_coolant', torch.tensor(config.temp_coeff_coolant, device=device, dtype=dtype)) 
        self.register_buffer('alpha_void', torch.tensor(config.void_coeff, device=device, dtype=dtype)) 
    ...
    def compute_derivatives(self, T_f, T_c, T_co, power_profile):
        ...
        T_bulk = torch.minimum(T_co, self.T_sat) 
